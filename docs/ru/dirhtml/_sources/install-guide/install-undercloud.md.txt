...
* С узла развёртывания должны быть доступны используемые в развёртывании 
  **DNS сервера**, **NTP сервера** и **BMC адреса** для всех физических узлов. 
  Также это значит, что заранее должны быть продуманы DNS и NTP сервера 
  организации.
* С узла развёртывания должна быть доступна **Public API** сеть развёртываемого 
  облака для настройки стартовых admin домена, проекта и пользователя. Эта 
  сеть называется **External** в разделе 
  [настройка сетей](full-deployment-init/#id7) 
